...
let capturedRequests = new Set<string>(); // Кэш для предотвращения дублей
...
export function activate(context: vscode.ExtensionContext) {
    ...
    app.post('/', (req: Request, res: Response) => {
        ...
        if (req.body.type === 'api_request' && req.body.data) {
            ...
            try {
                ...
                vscode.window.showInformationMessage(
                    `💾 Данные сохранены ОДНОКРАТНО: ${timestamp}`
                );
            ...
            res.json({
                modifiedSystemPrompt: originalSystemPrompt,  // БЕЗ изменений
                modifiedMessages: data.messages,             // БЕЗ изменений  
                modifiedMetadata: {
                    ...data.metadata,
                    hookIntercepted: true,
    ...
    app.post('/api/hook', (req: Request, res: Response) => {
        ...
        if (data && data.systemPrompt) {
            ...
            // Показываем уведомление  
            vscode.window.showInformationMessage(
                `🎯 Перехвачено (NEW API): "${contentText.substring(0, 50)}..."`
            );
    ...
    let toggleAutoStartCommand = vscode.commands.registerCommand('roo-hook-transit.toggleAutoStart', async () => {
        ...
        vscode.window.showInformationMessage(
            `Автозапуск ${!current ? 'включен' : 'выключен'}`
        );
    });
